...
#Region "Jako strony WWW"
    Private Function WebPageForLogin(oLogin As ShareLogin) As String
        Dim sPage As String = $"Current pictures for {oLogin.login.ToString}"

        sPage = $"<html><head><title>{sPage}</title><meta charset='utf-8' /></head>" & vbCrLf & $"<body><h1>{sPage}</h1>"
        ...
        If guard < 0 Then
            sPage &= "<p>(zdjêæ jest wiêcej, ale ogranicza mnie limit)</p>"
        End If

        If Not byloCos Then
            sPage &= "<p>W buforze nie ma zdjêæ pasuj¹cych do Twojego loginu</p>"
        End If
