...
namespace FacebookApiSharp.API.Processors
{
  ...
  //"fetch_privacy_value_for_pending_approval_comment": true
        ...
        public async Task<string> UploadPhotoToAlbumAsync(long albumId, string caption, string picname, byte[] imageBytes)
        {
            var instaUri = UriCreator.GetUploadPhotoToAlbum(albumId);
            ...
            var requestContent = new MultipartFormDataContent()
                ...
                {new StringContent("false"), "\"no_story\""},
                { new StringContent("true"), "\"published\""},

                { new StringContent("8"), "\"source\""},
            ...
            requestContent.Add(imageContent, "farr", picname);    // "farr" 

            // .Get: pobiera liste zdjec w albumie
            var request = _httpHelper.GetDefaultRequest(HttpMethod.Post,
                instaUri, _deviceInfo);
#if !false
